        ...
        <div class="flexbox">
          <div class="chat-box">
            ...
            <div class="chat-box-footer">
              ...
              <button id="send" @click.prevent="sendMessage(); playSound('http://soundbible.com/mp3/Air Plane Ding-SoundBible.com-496729130.mp3');">
...
export default {
  ...
  methods: {
    initTurn: function () {
      // init timer
      socket.emit('initTurn', this.room)
      this.turns--
    },
    startTurn: function () {
      // display timer
      socket.on('startTurn', (word) => {
        this.word = word
        // index back to 0
        this.currPlayer = this.room.players[this.index]
      })
    },
    addScore: function () {
      // or timer finished
      if (this.message === this.word) {
        socket.emit('addScore', {
          room: this.room,
          currTurn: this.turn,
          winner: this.chats[this.chats.length - 1].username
        })
      }
      this.initTurn()
    },
    displayScore: function () {
      socket.on('displayScore', (obj) => {
